...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>キャンプ場投稿｜シーズン別キャンプ場リサーチ！</title>
    ...
    <body class = "bg-black text-white italic font-bold grid justify-items-center">
       <h1 class = "text-2xl my-4">キャンプ場投稿</h1>
      ...
      　    @csrf
         <div class = "grid justify-items-center space-y-4">
          ...
          　     <input class = "border rounded text-black" type="text" name="post[camp]" placeholder="～キャンプ場" value="{{old('post.camp')}}"/>
          　     <p class = "camp_error" style = "color:red">{{$errors->first('post.camp')}}</p>
          　   </div>
          　   <div class = "grid justify-items-center">
          　     <h2 class = "text-xl text-white">住所</h2>
          　     <input class = "border rounded text-black" type = "text" name = "post[address]" placeholder = "○○県○○市～" value = "{{old('post.address')}}"/>
          　     <p class = "address_error" style = "color:red">{{$errors->first('post.address')}}</p>
          　   </div>
          　   <div class = "grid justify-items-center">
          　     <h2 class = "text-xl text-green-500">概要</h2>
          　     <textarea class = "resize border rounded text-black" name="post[body]" placeholder="特徴・魅力など">{{old('post.body')}}</textarea>
          ...
          　   </div>
          　   <div class = "text-xl">
          　     <input class = "rounded-md bg-blue-500 hover:bg-yellow-500 p-1 duration-300" type="submit" value="投稿"/>
          　   </div>
            </form>
          　   <div class = "text-xl">
